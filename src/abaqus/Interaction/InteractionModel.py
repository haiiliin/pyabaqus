...
class InteractionModel(InteractionContactControlModel,
    ...
    def AcousticImpedance(
        ...
        self.interactions[name] = interaction = AcousticImpedance(
            name, createStepName, surface, definition, interactionProperty,
            nonreflectingType, radius, semimajorAxis, eccentricity,
            centerCoordinates, directionCosine)
        return interaction
    ...
    def ActuatorSensor(self, name: str, createStepName: str, point: Region,
        ...
        self.interactions[name] = interaction = ActuatorSensor(
            name, createStepName, point, interactionProperty,
            noCoordComponents, unsymm, noSolutionDepVar, userSubUel, dof,
            solutionDepVars)
        return interaction
    ...
    def CavityRadiation(self,
        ...
        self.interactions[name] = interaction = CavityRadiation(
            name, createStepName, surfaces, surfaceEmissivities, ambientTemp,
            blocking, blockingSurfaces, rangeOfView, surfaceReflection,
            viewfactorAccurTol, minInfinitesimalRatio, numPointsPerEdge,
            minLumpedAreaDS, cyclicSymmetry, cyclicImages, cyclicRotPt,
            cyclicRotEndPt, cyclicSymPt, periodicSymmetries, periodicImages_1,
            periodicImages_2, periodicImages_3, periodicSymAxis_1,
            periodicSymAxis_2, periodicSymPlane_1, periodicSymPlane_2,
            periodicSymPlane_3, periodicDistance_1, periodicDistance_2,
            periodicDistance_3, periodicSymZ, periodicDistZ,
            reflectionSymmetries, reflectionSymAxis_1, reflectionSymAxis_2,
            reflectionSymPlane_1, reflectionSymPlane_2, reflectionSymPlane_3,
            reflectionSymZ)
        return interaction
    ...
    def ConcentratedFilmCondition(
        ...
        self.interactions[name] = interaction = ConcentratedFilmCondition(
            name, createStepName, region, definition, nodalArea,
            explicitRegionType, interactionProperty, field, sinkTemperature,
            sinkAmplitude, filmCoeff, filmCoeffAmplitude, sinkFieldName,
            sinkDistributionType)
        return interaction
    ...
    def ConcentratedRadiationToAmbient(
        ...
        self.interactions[name] = interaction = ConcentratedRadiationToAmbient(
            name, createStepName, region, ambientTemperature,
            ambientTemperatureAmp, emissivity, nodalArea, explicitRegionType,
    ...
    def ContactExp(
        ...
        ContactPropertyAssignment = ContactPropertyAssignment(),
        surfaceThicknessAssignments:
        SurfaceThicknessAssignment = SurfaceThicknessAssignment(),
        surfaceOffsetAssignments:
        SurfaceOffsetAssignment = SurfaceOffsetAssignment(),
        surfaceFeatureAssignments:
        SurfaceFeatureAssignment = SurfaceFeatureAssignment(),
        smoothingAssignments: SmoothingAssignment = SmoothingAssignment(),
        surfaceCrushTriggerAssignments:
        SurfaceCrushTriggerAssignment = SurfaceCrushTriggerAssignment(),
        surfaceFrictionAssignments:
        SurfaceFrictionAssignment = SurfaceFrictionAssignment(),
        mainSecondaryAssignments:
        MainSecondaryAssignment = MainSecondaryAssignment(),
        ...
        self.interactions[name] = interaction = ContactExp(
            name, createStepName, useAllstar, globalSmoothing, includedPairs,
            excludedPairs, contactPropertyAssignments,
    ...
    def ContactStd(
        ...
        SurfaceBeamSmoothingAssignment = SurfaceBeamSmoothingAssignment(),
        surfaceVertexCriteriaAssignments:
        SurfaceVertexCriteriaAssignment = SurfaceVertexCriteriaAssignment(),
        mainSecondaryAssignments:
        MainSecondaryAssignment = MainSecondaryAssignment(),
        initializationAssignments:
        InitializationAssignment = InitializationAssignment(),
        stabilizationAssignments:
        StabilizationAssignment = StabilizationAssignment(),
        smoothingAssignments: SmoothingAssignment = SmoothingAssignment(),
        slidingTransitionAssignments:
        SlidingTransitionAssignment = SlidingTransitionAssignment(),
        slidingFormulationAssignments:
        SlidingFormulationAssignment = SlidingFormulationAssignment()
    ) -> ContactStd:
    ...
    def CyclicSymmetry(
        ...
        self.interactions[name] = interaction = CyclicSymmetry(
            name, createStepName, main, secondary, repetitiveSectors,
            axisPoint1, axisPoint2, extractedNodalDiameter,
            lowestNodalDiameter, highestNodalDiameter, excitationNodalDiameter,
            adjustTie, positionTolerance, positionToleranceMethod)
        return interaction

    def ElasticFoundation(self, name: str, createStepName: str,
                          surface: Region,
                          stiffness: float) -> ElasticFoundation:
    ...
    def ExpInitialization(self,
        ...
        self.interactions[name] = interaction = ExpInitialization(
            name, overclosureType, interferenceDistance, clearanceDistance,
            openingTolerance, overclosureTolerance, adjustNodalCoords,
    ...
    def FilmCondition(
            ...
            field: str = '',
            sinkFieldName: str = '',
            sinkDistributionType: SymbolicConstant = UNIFORM) -> FilmCondition:
        ...
        self.interactions[name] = interaction = FilmCondition(
            name, createStepName, surface, definition, interactionProperty,
            sinkTemperature, sinkAmplitude, filmCoeff, filmCoeffAmplitude,
            field, sinkFieldName, sinkDistributionType)
        return interaction
    ...
    def FluidCavity(self,
                    ...
                    checkNormals: Boolean = ON) -> FluidCavity:
        ...
        self.interactions[name] = interaction = FluidCavity(
            name, createStepName, cavityPoint, cavitySurface,
            interactionProperty, ambientPressure, thickness, useAdiabatic,
            checkNormals)
        return interaction
    ...
    def FluidCavityProperty(
        ...
        self.interactions[name] = interaction = FluidCavityProperty(
            ...
            capacityType, capacityTempDep, capacityDependencies, capacityTable)
        return interaction

    def FluidExchange(self,
                      name: str,
                      createStepName: str,
                      firstCavity: str,
                      interactionProperty: str,
        ...
        self.interactions[name] = interaction = FluidExchange(
            name, createStepName, firstCavity, interactionProperty, definition,
            secondCavity, exchangeArea)
        return interaction
    ...
    def FluidInflator(self,
                      name: str,
                      createStepName: str,
                      cavity: str,
                      interactionProperty: str,
                      inflationTimeAmplitude: str = '',
                      massFlowAmplitude: str = '') -> FluidInflator:
        ...
        self.interactions[name] = interaction = FluidInflator(
            name, createStepName, cavity, interactionProperty,
    ...
    def FluidInflatorProperty(
        ...
        self.interactions[name] = interaction = FluidInflatorProperty(
            name, definition, effectiveArea, tankVolume, dischargeCoefficient,
            dataTable, numFluids, mixtureType, inflationTime,
            fluidbehaviorName, massFraction)
        return interaction

    def IncidentWave(self,
                     name: str,
                     createStepName: str,
                     sourcePoint: Region,
                     ...
                     magnitudeFactor: float = 1) -> IncidentWave:
        ...
        self.interactions[name] = interaction = IncidentWave(
            name, createStepName, sourcePoint, standoffPoint, surface,
            interactionProperty, definition, amplitude, imaginaryAmplitude,
            surfaceNormal, initialDepth, referenceMagnitude, detonationTime,
            magnitudeFactor)
        return interaction
    ...
    def IncidentWaveProperty(
        ...
        self.interactions[name] = interaction = IncidentWaveProperty(
            ...
            specificHeatRatio, gravity, atmosphericPressure, dragCoefficient,
            dragExponent, waveEffects, chargeDensity, chargeMass, constantK1,
            constantK2, constantA, constantB, constantKc, duration,
            maximumSteps, relativeStepControl, absoluteStepControl,
            stepControlExponent, genDecayA, genDecayB, genDecayC, seedNumber,
            massTNT, massFactor, lengthFactor, timeFactor, pressureFactor)
        return interaction
    ...
    def ModelChange(self,
                    ...
                    includeStrain: Boolean = OFF) -> ModelChange:
        ...
        self.interactions[name] = interaction = ModelChange(
            name, createStepName, isRestart, regionType, region, activeInStep,
            includeStrain)
        return interaction

    def PressurePenetration(self,
                            name: str,
                            createStepName: str,
                            ...
                            amplitude: str = UNSET,
                            penetrationTime: float = 0) -> PressurePenetration:
        ...
        self.interactions[name] = interaction = PressurePenetration(
            name, createStepName, contactInteraction, mainPoints,
            secondaryPoints, penetrationPressure, criticalPressure, amplitude,
            penetrationTime)
        return interaction
    ...
    def RadiationToAmbient(
            ...
            ambientTemperatureAmp: str = '') -> RadiationToAmbient:
        ...
        self.interactions[name] = interaction = RadiationToAmbient(
            name, createStepName, surface, emissivity, field, distributionType,
            radiationType, ambientTemperature, ambientTemperatureAmp)
        return interaction

    def SelfContactExp(self,
                       name: str,
                       createStepName: str,
                       surface: Region,
                       interactionProperty: str,
                       mechanicalConstraint: SymbolicConstant = KINEMATIC,
                       contactControls: str = '') -> SelfContactExp:
        ...
        self.interactions[name] = interaction = SelfContactExp(
            name, createStepName, surface, interactionProperty,
            mechanicalConstraint, contactControls)
        return interaction

    def SelfContactStd(self,
                       name: str,
                       createStepName: str,
                       surface: Region,
                       interactionProperty: str,
                       ...
                       smooth: float = 0,
                       contactControls: str = '') -> SelfContactStd:
        ...
        self.interactions[name] = interaction = SelfContactStd(
            name, createStepName, surface, interactionProperty, enforcement,
            thickness, smooth, contactControls)
        return interaction
    ...
    def StdInitialization(
            ...
            clearanceDistance: float = None,
            openingTolerance: float = None,
            overclosureTolerance: float = None) -> StdInitialization:
        ...
        self.interactions[name] = interaction = StdInitialization(
            name, overclosureType, interferenceDistance, clearanceDistance,
            openingTolerance, overclosureTolerance)
        return interaction

    def StdStabilization(self,
                         name: str,
        ...
        self.interactions[name] = interaction = StdStabilization(
            name, zeroDistance, reductionFactor, scaleFactor, tangentialFactor,
            amplitude, reset)
        return interaction
    ...
    def StdXplCosimulation(
        ...
        self.interactions[name] = interaction = StdXplCosimulation(
            name, createStepName, region, incrementation, stepSize,
            stepSizeDefinition)
        return interaction
    ...
    def SurfaceToSurfaceContactExp(
        ...
        self.interactions[name] = interaction = SurfaceToSurfaceContactExp(
            name, createStepName, main, secondary, sliding,
            interactionProperty, mechanicalConstraint, weightingFactorType,
            weightingFactor, contactControls, initialClearance,
            halfThreadAngle, pitch, majorBoltDiameter, meanBoltDiameter,
            datumAxis, useReverseDatumAxis, clearanceRegion)
        return interaction
    ...
    def SurfaceToSurfaceContactStd(
        ...
        self.interactions[name] = interaction = SurfaceToSurfaceContactStd(
            name, createStepName, master, slave, sliding, interactionProperty,
            interferenceType, overclosure, interferenceDirectionType,
            direction, amplitude, smooth, hcrit, extensionZone, adjustMethod,
            adjustTolerance, adjustSet, enforcement, thickness,
            contactControls, tied, initialClearance, halfThreadAngle, pitch,
            majorBoltDiameter, meanBoltDiameter, datumAxis,
            useReverseDatumAxis, clearanceRegion, surfaceSmoothing, bondingSet,
            handedness, normalAdjustment)
        return interaction

    def XFEMCrackGrowth(self,
                        name: str,
                        createStepName: str,
                        crackName: str,
